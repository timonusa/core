...
abstract class Unit
{
    ...
    {
        $lines = [];
        $sql = static::getPDO()->prepare('SELECT * FROM ' . static::TABLE_NAME .' ORDER BY id DESC ' );
        $sql->execute();
     ...
     * @return int
     */
    public static function createLine(array $fields,array $values) : ?int
    {
        $fields_str = '';
    ...
    {
        $sql = static::getPDO()->prepare('SELECT COUNT(*) as num FROM ' . static::TABLE_NAME . ' WHERE id=' . $this->id);
        $sql->execute();
        if ($sql->fetch(\PDO::FETCH_LAZY)->num > 0 ) {
            return true;
        }
        return false;
    }
